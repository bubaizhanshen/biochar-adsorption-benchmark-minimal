...
def main() -> None:
    ...
    panels.to_csv(args.out_dir / "evidence_by_panel.csv", index=False)
    primary_panels.to_csv(
        args.out_dir / "primary_panel_evidence.csv", index=False
    )
    task_summary.to_csv(args.out_dir / "evidence_by_task.csv", index=False)
    overall_rows = []
    for label, subset in [
    ...
    pd.DataFrame(overall_rows).to_csv(
        args.out_dir / "overall_summary.csv", index=False
    )
    schema.to_csv(args.out_dir / "data_template.csv", index=False)
    readme = [
    ...
    print(args.out_dir / "evidence_by_task.csv")
