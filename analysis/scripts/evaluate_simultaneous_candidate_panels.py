...
SUPPORT_DIR = ROOT / "analysis/work/candidate_panel_support"
DEFAULT_MANIFEST = ROOT / "analysis/results/candidate_panels/manifest.csv"
DEFAULT_SHARDS = ROOT / "analysis/work/candidate_panels/full_model_shards"
DEFAULT_OUT = ROOT / "analysis/results/candidate_panels/full_model"
...
def merge_shards(
    ...
    searches.to_csv(out_dir / "model_candidates.csv", index=False)
    cells_output.to_csv(out_dir / "matched_material_cells.csv", index=False)
    strata_output.to_csv(out_dir / "ranking_by_condition.csv", index=False)
    panel_summary.to_csv(out_dir / "panel_summary.csv", index=False)
    task_source_summary.to_csv(
        out_dir / "summary_by_task_study_block.csv", index=False
    )
    task_summary.to_csv(out_dir / "summary_by_task.csv", index=False)
    pd.DataFrame([overall, conservative]).to_csv(
        out_dir / "overall_summary.csv", index=False
    )
