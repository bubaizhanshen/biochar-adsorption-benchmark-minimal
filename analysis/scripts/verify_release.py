...
def verify_material_holdout() -> dict[str, float | int]:
    ...
    summary = pd.read_csv(directory / "task_summary.csv")
    predictions = pd.read_csv(directory / "oof_predictions.csv")
    diagnostics = pd.read_csv(directory / "fold_diagnostics.csv")
    candidates = pd.read_csv(directory / "model_candidates.csv")
...
def verify_study_holdout(
    ...
    directory = RESULTS / "holdout" / directory_name
    summary = pd.read_csv(directory / "task_summary.csv")
    predictions = pd.read_csv(directory / "oof_predictions.csv")
    diagnostics = pd.read_csv(directory / "fold_diagnostics.csv")
    candidates = pd.read_csv(directory / "model_candidates.csv")
...
def verify_common_weighting() -> dict[str, float | int]:
    paired = pd.read_csv(
        RESULTS / "holdout/common_weighting/paired_comparison.csv"
    )
    assert len(paired) == 6
...
def verify_candidate_panels() -> dict[str, float | int]:
    benchmark = RESULTS / "candidate_panels"
    evidence = benchmark / "evidence"
    manifest = pd.read_csv(benchmark / "manifest.csv")
    full_diagnostics = pd.read_csv(benchmark / "full_model/diagnostics.csv")
    condition_diagnostics = pd.read_csv(
        benchmark / "condition_only_model/diagnostics.csv"
    )
    panels = pd.read_csv(evidence / "evidence_by_panel.csv")
    primary = pd.read_csv(evidence / "primary_panel_evidence.csv")
    tasks = pd.read_csv(evidence / "evidence_by_task.csv")
    overall = pd.read_csv(evidence / "overall_summary.csv")
...
def verify_locked_application() -> dict[str, float | int]:
    ...
    queries = pd.read_csv(directory / "condition_results.csv")
...
def main() -> None:
    ...
    material_inner = verify_study_holdout(
        "inner_grouping_sensitivity", "material", 0.3144349794790323, 5, 2
    )
