...
RESULTS = ROOT / "reanalysis/results/merged_ibuprofen_benchmark"
MATERIAL_OOF = (
    RESULTS / "material_benchmark_10_tasks/traceable_10_task_oof_predictions.csv"
)
STUDY_OOF = (
    RESULTS / "source_benchmark_10_tasks/source_study_holdout_oof_predictions.csv"
)
OUTPUT = ROOT / "reanalysis/results/holdout_common_weighting"
...
def main() -> None:
    ...
    combined.to_csv(args.out_dir / "holdout_common_weighting_long.csv", index=False)
    paired.to_csv(args.out_dir / "holdout_common_weighting_paired.csv", index=False)
    ...
    (args.out_dir / "HOLDOUT_COMMON_WEIGHTING.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(args.out_dir / "holdout_common_weighting_paired.csv")
    print("\n".join(lines))
