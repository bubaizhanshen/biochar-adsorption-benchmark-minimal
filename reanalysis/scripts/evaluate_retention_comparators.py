...
INPUT = (
    ROOT
    / "reanalysis/external_sources/new_source_panels/postfreeze_v1_locked_panels_combined.csv"
)
...
def main() -> None:
    ...
    panel_results.to_csv(
        OUT / "retention_equal_budget_comparators_by_panel.csv", index=False
    )
    anchor_results.to_csv(OUT / "retention_all_anchor_pairs.csv", index=False)
    summaries.to_csv(
        OUT / "retention_equal_budget_comparators_summary.csv", index=False
    )
