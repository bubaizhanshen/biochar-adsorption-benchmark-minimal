...
REGISTRY_DIR = ROOT / "reanalysis" / "registries"
DEFAULT_MANIFEST = ROOT / "reanalysis" / "results" / "material_holdout_manifest.csv"
DEFAULT_SHARDS = ROOT / "reanalysis" / "results" / "material_holdout_shards"
DEFAULT_OUT = (
    ROOT
    / "reanalysis"
    / "results"
...
def merge_shards(
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    predictions.to_csv(out_dir / "traceable_10_task_oof_predictions.csv", index=False)
    diagnostics.to_csv(out_dir / "traceable_10_task_fold_diagnostics.csv", index=False)
    candidates.to_csv(out_dir / "traceable_10_task_model_candidates.csv", index=False)
    summary.to_csv(out_dir / "traceable_10_task_summary.csv", index=False)
    ...
    print(out_dir / "traceable_10_task_summary.csv")
